    ...
    <header className="grid sticky top-0 left-0 z-50 bg-[#EFEFEF]">
      ...
      <div className="flex md:items-center md:p-5 md:justify-center">
        <NavLink prefetch="intent" to="/" style={activeLinkStyle} end>
          <img
            src={logo_WeW}
            className="w-[120px] md:w-[200px] m-3 md:mx-16"
      ...
      {items.map((item, i) => {
        ...
        return (
          <React.Fragment key={item.id}>
            <NavLink
              className="header-menu-item font-sans font-bold hover:text-theme-mauve-fonce text-md"
              end
              onClick={close}
              prefetch="intent"
              style={activeLinkStyle}
...
function HeaderCtas({isLoggedIn, cart}) {
  return (
    <nav className="header-ctas mx-3 md:mx-16" role="navigation">
...
function HeaderMenuMobileToggle() {
  ...
  return (
    <button
      ...
      onClick={() => open('mobile')}
    >
      <RiMenuFill size={28} className="cursor-pointer" />
    </button>
  );
}
